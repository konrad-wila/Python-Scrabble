...
def get_letter_score(letter, scores):
...
def get_word_score(word, scores):
    ...
    score = 0
    for letter in word:
        score += get_letter_score(letter, scores)
    return score

def can_be_made(word, tiles):
...
def remove_word_from_tiles(word, tiles):
    """Removes the word from the tiles."""
    for letter in word:
        tiles.remove(letter)
    return tiles

def word_input(tiles, dictionary, scores):
    ...
    remove_word_from_tiles(word, tiles)
...
def tile_score(tiles, scores):
    ...
    score = []
    for letter in tiles:
        score.append(get_letter_score(letter, scores))
...
def start_game():
    ...
    word_input(playerTiles, dictionary, scores)
